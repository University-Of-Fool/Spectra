...
pub async fn main_service(
    ...
    // 如果站点未正确安装，拦截所有非 /setup/ 的请求
    if !state.runtime_config.load().setup {
        let redirect_to_setup = (
            StatusCode::FOUND,
            [(header::LOCATION, "/setup/".to_string())],
        )
            .into_response();

        // 如果请求的路径是以下路径之一，则直接返回 302 to /setup/
        // 在末尾加一个 /：为了避免有些时候访问路径末尾不带 / 导致匹配失败（比如访问 /dashboard 而不是 /dashboard/）
        // 同时避免像是 /dashboard-something 这样的路径被误判为访问了 /dashboard/
        static PAGES: [&str; 5] = ["dashboard/", "code/", "password/", "not_found/", "admin/"];
        let path = format!("{}/", request.uri().path().trim_start_matches('/'));
        if path == "/" || PAGES.iter().find(|&&p| path.starts_with(p)).is_some() {
    ...
    let request = if let Some(password) = item.password_hash.clone() {
        ...
        let query = axum::extract::Query::<PasswordForm>::from_request_parts(&mut parts, &()).await;
            ...
            if let Some(mut file) = state.file_accessor.get_file(item_data_clone.clone()).await {
                ...
                if item.extra_data.is_none() {
                    if let Some(origin) = request.headers().get("Origin") {
                        response_builder =
                            response_builder.header("Access-Control-Allow-Origin", origin.clone());
                    }
                }
